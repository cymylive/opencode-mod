...
import { getDisplayTitle, isDefaultTitle } from "../../util/session"
...
export function Sidebar(props: { sessionID: string; overlay?: boolean; onClose?: () => void }) {
  ...
  const history = createMemo(() => {
    ...
    const limit = showAllHistory() ? 50 : 20
    return sync.data.session
      .filter((s) => s.directory === dir && !s.parentID)
      .toSorted((a, b) => {
        const aPinned = local.session.isPinned(a.id)
        const bPinned = local.session.isPinned(b.id)
        if (aPinned !== bPinned) return aPinned ? -1 : 1
        return (b.time.updated ?? 0) - (a.time.updated ?? 0)
      })
      .slice(0, limit)
  })
  const totalHistory = createMemo(() => {
    const current = session()
    if (!current) return 0
    return sync.data.session.filter((s) => s.directory === current.directory && !s.parentID).length
  })
              ...
              <box paddingRight={1}>
                <box flexDirection="row" justifyContent="space-between">
                  ...
                  <Show when={props.onClose}>
                    <box paddingX={1} onMouseUp={props.onClose}>
            ...
            <pluginRuntime.Slot name="sidebar_content" session_id={props.sessionID}>
              <Show when={history().length > 0}>
                <box paddingTop={2} gap={1}>
                        ...
                        <box gap={0} paddingLeft={1}
                          ...
                          <box flexDirection="row" alignItems="center" gap={1}>
                              ...
                              <text fg={isCurrent(s.id) ? theme.primary : theme.text} wrapMode="ellipsis" width={26}>
                                {(() => {
                                  const title = getDisplayTitle(
                                    s,
                                    sync.data.message[s.id],
                                    sync.data.part as any,
                                    extractedTitles(),
                                  )
                                  if (title && !extractedTitles()[s.id] && title !== s.title && isDefaultTitle(s.title)) {
                                    setExtractedTitles((prev) => ({ ...prev, [s.id]: title }))
                                  }
                            ...
                            <Show when={!isCurrent(s.id)}>
                              <box paddingX={1} onMouseUp={() => local.session.togglePin(s.id)}>
                                <text fg={local.session.isPinned(s.id) ? theme.warning : theme.textMuted}>
                                  {local.session.isPinned(s.id) ? "★" : "☆"}
                                </text>
                              </box>
                  ...
                  <Show when={!showAllHistory() && totalHistory() > 20}>
                    <box paddingLeft={4} onMouseUp={() => setShowAllHistory(true)}>
                      <text fg={theme.textMuted}><i>+ Show more ({totalHistory() - 20} more)</i></text>
                    </box>
                  </Show>
                </box>
              </Show>
