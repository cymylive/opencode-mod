      ...
      >
        <box flexDirection="row" flexGrow={1} minHeight={0}>
          <box flexGrow={1} minHeight={0} paddingBottom={1} paddingLeft={2} paddingRight={2} gap={1}
            onMouseUp={() => { if (sidebarVisible()) { setSidebar(() => "hide"); setSidebarOpen(false) } }}
          >
            <Show when={session()}>
              <scrollbox
                ref={(r) => (scroll = r)}
          ...
          <Show when={sidebarVisible()}>
            <Sidebar sessionID={route.sessionID} onClose={() => setSidebarOpen(false)} />
          </Show>
        </box>
          </context.Provider>
        </LocationProvider>
  ...
  return (
    <>
      ...
      <Show when={props.message.error && props.message.error.name !== "MessageAbortedError"}>
        ...
        >
          <text fg={theme.textMuted}>{props.message.error?.data?.message ?? errorMessage(props.message.error)}</text>
        </box>
      </Show>
      <Switch>
