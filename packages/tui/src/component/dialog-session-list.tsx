    ...
    function buildOption(id: string, category: string) {
      ...
      const gutter = isWorking
        ...
        : slot !== undefined
          ...
          : undefined
      const displayTitle = isDeleting
        ? `Press ${deleteHint()} again to confirm`
        : getDisplayTitle(x, sync.data.message?.[x.id], sync.data.part as any, {}) ?? x.title
      if (isDefaultTitle(x.title) && !sync.data.message?.[x.id]?.length) {
        sync.session.sync(x.id).catch(() => {})
      }
      return {
        title: displayTitle,
